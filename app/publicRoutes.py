...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///'+os.path.join("database",'database.db')
...
def test():
    if request.method == "POST":
        ...
        det=Details(email=em,vidName=os.path.join(app.config['UPLOAD_FOLDER'],filename))
        db.session.add(det);
        db.session.commit();
        print("Added successfully");
        
        subs=textExtractor(filename);#here filename has extension mp4 or whatever but not wav.
        print(subs)
